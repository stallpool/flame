...
function set_cookie(key, value) {
   document.cookie = key + '=' + escape(value) + ';path=/;domain=' + window.flameenv.hostname;
}
...
function update_uuid(jsonData) {
   ...
   if (obj.sessionId) set_cookie('flame_uuid', obj.sessionId);
}
...
function login_and_start(redirect_url, env, before_init, init_app) {
   ...
   env = env || {};
   env.user = {
      user: cookie.flame_user || '',
      uuid: cookie.flame_uuid || ''
   };
